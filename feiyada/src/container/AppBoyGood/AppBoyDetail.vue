<template>
    <div class="bg-detail ">
        ...
        <div class="prodect_detail">
            ...
            <div class="counts">数量：
                <button v-on:click="counts-=1" @click="reduceGood({goodid:detail.id})">-</button>
    ...
    export default {
        ...
        methods:{             
            getDetailData(){
                ...
                axios.get('./static/json/'+that.$route.params.types+'.json').then((res)=>{
                    
                    //  for(var i in res.data){
                    //     res.data[i].forEach((item,j)=>{
                    //         var m = j+1
                    //         item.id=i+m
                    //     })
                    //  }
                    // console.log(res.data)                  
                     for(var k in res.data){
                         console.log(k)
                         if(k==localStorage['b_id'] ){
                           // console.log(res.data[k])
                            that.detail = res.data[k][that.$route.params.id]
                        }
                     }
                     //console.log(that.$route.params.id) 
                      for(var j in res.data){
                         if(j==localStorage['g_id']){
                           // console.log(res.data[j])
                            that.detail = res.data[j][that.$route.params.id]
                        }
                     }                              
                })
            },
